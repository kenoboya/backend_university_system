...
BEGIN
   IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'application_status') THEN
      EXECUTE 'DROP TYPE application_status';
   END IF;
END $$;

CREATE TYPE application_status AS ENUM (
    'accepted', 'consideration', 'denied'
);

DROP TABLE IF EXISTS applications_people;

CREATE TABLE applications_people (
    application_id INTEGER GENERATED BY DEFAULT AS IDENTITY,
    user_id INTEGER NOT NULL,
    role user_role NOT NULL,
    status application_status DEFAULT 'consideration',
    CONSTRAINT pk_applications_people_application_id PRIMARY KEY(application_id),
    CONSTRAINT fk_applications_people_user FOREIGN KEY (user_id) REFERENCES users(user_id)
);
...
DO $$
BEGIN
   IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'educational_level') THEN
      EXECUTE 'DROP TYPE educational_level';
   END IF;
END $$;
...
BEGIN
   IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'subject_type') THEN
      EXECUTE 'DROP TYPE subject_type';
   END IF;
END $$;

CREATE TYPE subject_type AS ENUM (
    'selected', 'general', 'specialized'
);
...
DROP TABLE IF EXISTS faculties_employees;
...
DROP TABLE IF EXISTS block_users_audit;
...
CREATE TABLE students_subjects (
    ...
    subject_id INTEGER REFERENCES subjects(subject_id),
    CONSTRAINT ck_students_subjects PRIMARY KEY(student_id, subject_id)
);

DO $$
BEGIN
   IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'attendance_records_status') THEN
      EXECUTE 'DROP TYPE attendance_records_status';
   END IF;
END $$;

CREATE TYPE attendance_records_status AS ENUM (
    'present', 'absent'
);
...
CREATE TABLE attendance_grades (
    ...
    student_id INTEGER REFERENCES students(student_id),
    status attendance_records_status DEFAULT 'absent',
    grade SMALLINT,
    CONSTRAINT ck_attendance_grades PRIMARY KEY(lesson_id, student_id)
);
...
CREATE TABLE news (
    ...
    CONSTRAINT fk_news_people FOREIGN KEY(author_id) REFERENCES people(person_id)
);
