...
func (r *StudentsRepository) GetStudentBriefInfoById(ctx context.Context, id uint64) (model.StudentBriefInfo, error) {
	...
	err := r.db.Get(&student, "SELECT name, surname, birth_date, photo FROM students JOIN people USING(person_id) WHERE student_id=$1", id)
	if err != nil {
		return student, err
	}
	return student, nil
...
func (r *StudentsRepository) GetStudentFullInfoById(ctx context.Context, id uint64) (model.StudentFullInfo, error) {
	...
	err := r.db.Get(&student, "SELECT name, surname, birth_date, photo, phone, address FROM students JOIN people USING(person_id) WHERE student_id=$1", id)
	if err != nil {
		return student, err
	}
	return student, nil
...
func (r StudentsRepository) GetStudentsAttendanceByLessonID(ctx context.Context, lesson_id uint64) ([]model.AttendanceRecord, error) {
	...
	err := r.db.Select(&attendanceRecords, "SELECT student_id, lesson_id, status FROM attendance_grades WHERE lesson_id = $1", lesson_id)
	if err != nil {
